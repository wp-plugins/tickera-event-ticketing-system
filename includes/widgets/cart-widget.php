...
class TC_Cart_Widget extends WP_Widget {
	...
	function TC_Cart_Widget() {
		...
		$this->WP_Widget( 'TC_Cart_Widget', __( 'Tickets Cart', 'tc' ), $widget_ops );
	}
					...
					<ul class='tc_cart_ul'>
						...
						foreach ( $cart_contents as $ticket_type => $ordered_count ) {
							...
							<li id='tc_ticket_type_<?php echo $ticket_type; ?>'>
								<?php echo apply_filters( 'tc_cart_widget_item', ($ordered_count . ' x ' . $ticket->details->post_title . ' (' . $tc->get_cart_currency_and_format( $ticket->details->price_per_ticket * $ordered_count ) . ')' ), $ordered_count, $ticket->details->post_title, $ticket->details->price_per_ticket ); ?>
							</li>
							<?php
						}
						?>
