...
class tc_event_date_time_element extends TC_Ticket_Template_Elements {
	...
	function ticket_content( $ticket_instance_id = false, $ticket_type_id = false ) {
		if ( $ticket_instance_id ) {
			...
			return apply_filters( 'tc_event_date_time_element_ticket_type' , date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( get_post_meta( $event_id, 'event_date_time', true ) ) ), $ticket_instance->details->ticket_type_id, $ticket_instance_id);
		} else {
			if ( $ticket_type_id ) {
